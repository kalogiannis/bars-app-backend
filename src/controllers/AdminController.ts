...
const createBarOwnerBar = async (req: Request, res: Response) => {
  try {
    const barOwnerId = req.params.id;
    const existingBar = await Bar.findOne({ user: barOwnerId });
    ...
    bar.user = new mongoose.Types.ObjectId(barOwnerId);
    ...
    console.error("Error creating bar for owner:", error);
